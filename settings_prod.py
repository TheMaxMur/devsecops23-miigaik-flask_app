...
class ProdConfig:
    ...
    SECRET_KEY = json.loads(requests.get(BASE_URL + '/v1/secret/prod/secret_key', \
                            headers={"X-Vault-Token": TOKEN}).text)['data']['SECRET_KEY']
    PASSWORD_SALT = json.loads(requests.get(BASE_URL + '/v1/secret/prod/password_salt', \
                            headers={"X-Vault-Token": TOKEN}).text)['data']['PASSWORD_SALT']
    
    DB_NAME = json.loads(requests.get(BASE_URL + '/v1/secret/prod/db_name', \
                            headers={"X-Vault-Token": TOKEN}).text)['data']['DB_NAME']

    DB_USER = json.loads(requests.get(BASE_URL + '/v1/secret/prod/db_user', \
                            headers={"X-Vault-Token": TOKEN}).text)['data']['DB_USER']

    DB_PASSWORD = json.loads(requests.get(BASE_URL + '/v1/secret/prod/db_password', \
    ...
    ADMIN = {
        'username': json.loads(requests.get(BASE_URL + '/v1/secret/prod/admin_username', \
                            headers={"X-Vault-Token": TOKEN}).text)['data']['ADMIN_USERNAME'],
        'password': json.loads(requests.get(BASE_URL + '/v1/secret/prod/admin_password', \
                            headers={"X-Vault-Token": TOKEN}).text)['data']['ADMIN_PASSWORD'],
        'email': json.loads(requests.get(BASE_URL + '/v1/secret/prod/admin_email', \
